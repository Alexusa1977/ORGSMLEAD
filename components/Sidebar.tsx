...
import { KeywordFile, Lead, PlatformConnection } from '../types';
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const platforms = [
    ...
    { name: 'Nextdoor', id: 'nextdoor', icon: 'n' },
  ...
  const isConnected = (platformId: string) => {
    return connections.find(c => c.platform === platformId)?.isConnected;
  };
    ...
    <aside className="w-72 bg-white border-r border-slate-200 flex flex-col h-full shadow-sm z-10">
      <div className="p-6">
        <div className="flex items-center gap-3 mb-10">
          ...
          <span className="font-bold text-slate-800 tracking-tight text-xl">LeadSync AI</span>
        </div>
        ...
        <div className="mb-8">
          <p className="px-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Monitor Folders</p>
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-2 rounded-xl text-sm transition-all flex items-center justify-between ${
                    activePlatform === p.id ? 'bg-slate-100 text-slate-900 font-bold' : 'text-slate-500 hover:bg-slate-50'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <span className={`w-5 h-5 rounded flex items-center justify-center text-[10px] font-bold uppercase ${connected ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : 'bg-slate-100'}`}>{p.icon}</span>
                    {p.name}
                  </div>
                  <div className="flex items-center gap-1.5">
                    {connected && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>}
              ...
              <div key={file.id} className="group relative">
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 rounded-xl text-sm transition-all flex items-center justify-between ${
                    activeFileId === file.id && activeView === 'collection' ? 'bg-indigo-50/50 text-indigo-700 font-bold' : 'text-slate-500 hover:bg-slate-50'
                  }`}
                >
      ...
      <div className="mt-auto p-6 border-t border-slate-100">
        <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100">
          <div className="w-10 h-10 rounded-full bg-indigo-100 border border-indigo-200 flex items-center justify-center text-indigo-700 font-bold shadow-sm">DS</div>
          <div className="min-w-0">
            <p className="text-sm font-bold text-slate-700 truncate">Danish Soomro</p>
            <p className="text-[10px] text-slate-400 font-bold uppercase">Pro Monitor</p>
          </div>
        </div>
      </div>
    </aside>
